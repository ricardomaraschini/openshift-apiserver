...
type ImporterFunc func(secrets []corev1.Secret, regConf *sysregistriesv2.V2RegistriesConf) importer.Interface
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, options *metav1.CreateOptions) (runtime.Object, error) {
	...
	// XXX
	secs := make([]corev1.Secret, 0)
	nsSecrets, err := r.isV1Client.ImageStreams(namespace).Secrets(isi.Name, metav1.GetOptions{})
	if err != nil {
		klog.Infof("error loading namespace secrets: %v", err)
	} else {
		secs = nsSecrets.Items
	}

	imports := r.importFn(secs, v2regConf)
	...
	if importFailed {
		imports := r.importFn(nil, v2regConf)
		if err := imports.Import(ctx.(gocontext.Context), isi, stream); err != nil {
			return nil, kapierrors.NewInternalError(err)
		}
	}
