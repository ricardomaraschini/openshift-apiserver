...
func (iptr *ImageStreamImporter) Import(ctx context.Context, isi *imageapi.ImageStreamImport, stream *imageapi.ImageStream) error {
	...
	iptr.importImages(ctx, isi, stream)
	iptr.importFromRepository(ctx, isi)
	return nil
}
...
// set.
func (iptr *ImageStreamImporter) importFromRepository(ctx context.Context, isi *imageapi.ImageStreamImport) {
...
func (iptr *ImageStreamImporter) getManifestFromSource(
	...
	repo, err := iptr.retriever.Repository(ctx, imageRef, insecure)
	if err != nil {
...
// V2RegistriesConf.
func (iptr *ImageStreamImporter) getManifest(
	ctx context.Context, ref reference.Named, insecure bool,
) (distribution.Manifest, distribution.ManifestService, distribution.BlobStore, error) {
	var errs []error
	...
	for _, pullSource := range pullSources {
		...
		manifest, ms, bs, err := iptr.getManifestFromSource(ctx, pullSource.Reference, insecure)
		if err != nil {
...
func (iptr *ImageStreamImporter) importManifest(
	ctx context.Context,
...
func (iptr *ImageStreamImporter) importRepositoryFromDocker(ctx context.Context, repository *importRepository) {
	...
	for i := range repository.Digests {
		...
		manifest, ms, bs, err := iptr.getManifest(ctx, dockerRef, repository.Insecure)
		if err != nil {
	...
	if count := repository.MaximumTags; count > 0 || count == -1 {
		...
		repo, err := iptr.retriever.Repository(ctx, repository.Ref, repository.Insecure)
		if err != nil {
			klog.V(5).Infof("unable to access repository %#v: %#v", repository, err)
			if strings.HasSuffix(err.Error(), "does not support v2 API") {
				importRepositoryFromDockerV1(ctx, repository, iptr.limiter)
				return
			}
